...
class_names = ["90 degree", "Entry_point", "Green", "T-intersection", "X-intersection", "None"]
...
def balls(class_names_evaczone, button, buttonL, buttonR, ser, displayGUI, Inference, get_picture, send_secret_data, runMotors, driveTowardsBall, pointTurnIntersection, forwardDrive, frame_height, frame_width, ballIndex):
    ...
    balls = ["Live victim", "Live victim", "Dead victim", "None"]
    temp = ballIndex
    while temp == ballIndex:
            ...
            for contour in contours_black:
                if cv2.contourArea(contour) > 50000:
                    ...
                    if w > 0.8 * frame.shape[1] and h > 0.2 * frame.shape[0]:
                        ...
                        pointTurnIntersection(get_picture, -50)
                        forwardDrive(get_picture, 100)
                        time.sleep(1)
                        send_secret_data(3, 5)
                        exited = True
...
ballsDone = 0
reverseIntersection = False
reverseIntersectionFirstTime = False
while True:
    if exited and buttonR.is_pressed and buttonL.is_pressed:
        ret, frame = get_picture(0)
        if ret:
            # Crop to upper half and middle 50% in x
            h, w = frame.shape[:2]
            x_start = int(w * 0.25)
            x_end = int(w * 0.75)
            y_start = 0
            ...
            # Define black range in HSV
            lower_black = np.array([0, 0, 0])
            upper_black = np.array([180, 255, 50])

            # Create black mask
            black_mask = cv2.inRange(hsv_cropped, lower_black, upper_black)
            # White mask is the inverse of black mask
            white_mask = cv2.bitwise_not(black_mask)

            displayGUI(None, black_mask)
            ...
            if white_pixels > black_pixels:
                ...
                ballsDone = ballsDone + 1
                exited = False
            else:
                reverseIntersection = True
                forwardDrive(get_picture, -100)
                pointTurnIntersection(get_picture, -180)
                reverseIntersectionFirstTime = True

            
        
    
    if exited and buttonR.is_pressed:
        runMotors(20, 10)
        time.sleep(0.3)

    if exited and buttonL.is_pressed:
        runMotors(-20, 10)
        time.sleep(0.3)
    ...
    if objectDetected:
            ...
            if (y_end - y_start) > 120 and (class_names[closest_class] != "Entry_point"):
                ...
                if class_names[closest_class] == "90 degree":  # 90 Degree
                    ...
                    if object_width >= 0.7 * frame_width and object_height <= 0.7 * frame_height:  # Check width and height
                        image_height, image_width, start, frame, annotated_image, detections, exited = balls(class_names_evaczone, button, buttonL, buttonR, ser, displayGUI, Inference, get_picture, send_secret_data, runMotors, driveTowardsBall, pointTurnIntersection, forwardDrive, frame_height, frame_width, ballsDone)
                        hasExited = True           
                    ...
                    if TintersectionOkay:
                        if (right and left) or (reverseIntersection and not reverseIntersectionFirstTime):
                            print("U-TURN")
                        ...
                        if reverseIntersectionFirstTime:
                            forwardDrive(get_picture, 100)
                            pointTurnIntersection(get_picture, 180)
                            reverseIntersectionFirstTime = False
        ...
        if contours_black:
            ...
            if w >= 0.95 * frame.shape[1] and h < 0.4 * frame.shape[0] and not exited:
                print("Biggest black contour is larger than 95% of the frame width")
                image_height, image_width, start, frame, annotated_image, detections, exited = balls(class_names_evaczone, button, buttonL, buttonR, ser, displayGUI, Inference, get_picture, send_secret_data, runMotors, driveTowardsBall, pointTurnIntersection, forwardDrive, frame_height, frame_width, ballsDone)
