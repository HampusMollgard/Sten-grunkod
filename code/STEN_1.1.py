...
def filter_and_average_k(coefficients, latestLineCoefficient, threshold_degrees=70):
    ...
    for k_value in coefficients:
        # Calculate angle between latestLineCoefficient and current k_value
        angle = math.atan(abs((k_value - latestLineCoefficient) / (1 + k_value * latestLineCoefficient)))
        
        # Keep coefficients that are within the threshold
        if angle <= threshold_radians:
            filtered_coefficients.append(k_value)
    
    # Calculate the average of the filtered k values
    if filtered_coefficients:
        average_k = sum(filtered_coefficients) / len(filtered_coefficients)
    else:
        average_k = None  # No coefficients left after filtering
    
    return filtered_coefficients, average_k



def find_lines_in_roi(image, point, roi_size=400):
    x, y = point
    ...
    x2 = min(x + half_size, image.shape[1])
    y2 = min(y + half_size, image.shape[0])

    # Create a mask with the same size as the image, only the ROI is set to 255
    ...
    # Loop through each contour and fit a line if it's large enough
    for contour in contours:
        if len(contour) >= 2:  # At least two points are needed to fit a line
            [vx, vy, x0, y0] = cv2.fitLine(contour, cv2.DIST_L2, 0, 0.01, 0.01)

            # Convert vector (vx, vy) to line slope k (vx / vy)
            if vy != 0:
                k = vx / vy
                line_coefficients.append(k)
