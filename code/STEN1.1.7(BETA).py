...
# Open the video
if devMode:
    cap = cv2.VideoCapture('TestRun2NewRatio.mp4')
        ...
        if x is not None: 
            ...
            while pos is not None and i < 20:
                ...
                if width is not None and width > 12 and tried_intersection is False:
                    ...
                    try:
                        ...
                        print(linearReg[0])
                        processed_frame, nIntersectedLines, intersection_point = get_intersection_data(processed_frame, mask, point[0], point[1], np.arctan(linearReg[0]), 100)

                    except:
                        processed_frame, nIntersectedLines, intersection_point = get_intersection_data(processed_frame, mask, point[0], point[1], angle, 100)
            ...
            if pos is None and y > 400 and turnAround is False and 0 < int(x + (np.sin(angle) * diagonal_distance)) < frame_width - 1 and 0 < int(y - (np.cos(angle) * diagonal_distance)) < frame_height - 1:
                try:
                    ...
                    continue
                processed_frame, nIntersectedLines, intersection_point = get_intersection_data(processed_frame, mask, x, y, np.arctan(linearReg[0]), 100)
