...
hef_path = '/home/malte/Downloads/best17.hef'
hef = HEF(hef_path)
...
def driveTowardsPoint(cX, cY, frame_height, frame_width, speed): 
    ...
    driveTowardsPoint.prevDrivingAngle = DrivingAngle
    if DrivingAngle > 0:
        DrivingAngle = abs(0.01*abs(DrivingAngle)+pow(1.08, abs(DrivingAngle) - 30)-abs(driveTowardsPoint.integral*kI)-(derivative*kD)) #possible att stalla upp reaktiviteten
    else:
...
def pointTurn(get_picture, angle):
    runMotors(math_sign(angle) * 0.1, 1)
    time.sleep(0.3)
    runMotors(math_sign(angle) * 0.1, 10)
    time.sleep(0.3)
    start_time = time.time()
    send_secret_data(4, 90)
    turning = True
    while turning:
        if ser.in_waiting > 0:
            response = ser.readline().decode().strip()
    ...
    if not turning:
        pointTurnIntersection(get_picture, -math_sign(angle) * 160)
...
while True:
    ...
    if contours:
        ...
        if len(sorted_contours) > 1:
            ...
            dist = 100    # maximum distance in px to consider "bordering"

            if cv2.contourArea(second_largest_contour) > area:
                # Calculate the minimum distance between the two contours
                min_dist_contour = np.min([cv2.norm(p1 - p2) for p1 in largest_contour[:, 0] for p2 in second_largest_contour[:, 0]])
                if min_dist_contour < dist:
                    # Concatenate the two contours and make a new one
                    merged = np.vstack((largest_contour, second_largest_contour))
                    merged = cv2.convexHull(merged)
                    largest_contour = merged
    ...
    if objectDetected:
        if closest_corner:
            ...
            x_start = int(max(0, (bottom_most_point[0] + 70) - width // 2))
            x_end = int(min(frame.shape[1], (bottom_most_point[0] + 70) + width // 2))
            y_start = int(max(0, 640 - vertical_distance))
            y_end = int(640)
                                ...
                                if confidence > 0.6:  # Filter low-confidence detections
                                    ...
                                    if box_middle_y > intersection_box_middle_y:  # Below the X-Intersection
                                        if box_middle_x < intersection_box_middle_x:
                                            ...
                                            right = True
                                            print("Right")
                    if TintersectionOkay and intersection_box_middle_x is not None:
                        if right and left:
                            print("U-TURN")
