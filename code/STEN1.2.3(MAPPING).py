...
def get_picture(dev_):
    ...
    if dev_:
        ...
        ret_ = True
        frame_ = cv2.rotate(frame_, cv2.ROTATE_180)
    return ret_, frame_ #ret = [have the image been read succsessfully], frame = [captured image from cam or vid]

def get_x_y_normal_line_follow(frame_height_, frame_width_, lastX_, forward_distance_, mask):
    x = None
...
def try_intersection(graph_x_, graph_y_, processed_frame_, mask_, point_, angle_, green_mask_, turnAround_, forward_distance_, frame_height_, frame_width_, pos_, frame_, x, y):
    try:
...
def normal_line_follow_pre_calc(frame):
    lastX = 500
    turnAround = False
    ...
    x, y = get_x_y_normal_line_follow(frame_height, frame_width, lastX, forward_distance, mask)
        ...
        while pos is not None and i < 8:
            ...
            if width is not None and width > 12 and tried_intersection is False:
                ...
                graph_x, graph_y, x, y, processed_frame, angle, turnAround = try_intersection(graph_x, graph_y, processed_frame, mask, point, angle, green_mask, turnAround, 
                                                                                    forward_distance, frame_height, frame_width, pos, frame, x, y)
...
def normal_line_follow(frame):
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    frame_height, frame_width = frame.shape[:2]
    
    processed_frame, x, y = normal_line_follow_pre_calc(frame)
...
def addToMap(canvas, frame, yaw, prevPoints, prevYaw, failedMapping, dx, dy):
    padding = 50  # Padding size
    ...
    rotation_angle = float(prevYaw) - float(yaw)
    prevPoints = points
    ...
    while True:
        ...
        if ret:
            ...
            if not devMode:
                out.write(frame_bgr)
            orientationLine += 3
            x, y, processed_frame = normal_line_follow(frame)
            result, canvas, prevPoints, failedMapping, dx, dy = addToMap(canvas, frame, yaw, prevPoints, prevYaw, failedMapping, dx, dy)
        else:
            errors += 1
