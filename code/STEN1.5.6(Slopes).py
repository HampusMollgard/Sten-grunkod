...
hef_path = '/home/malte/Downloads/best15.hef'
hef = HEF(hef_path)
...
def driveTowardsPoint(cX, cY, frame_height, frame_width, speed):
    ...
    driveTowardsPoint.prevDrivingAngle = DrivingAngle
    if DrivingAngle > 0:
        DrivingAngle = abs(0.01*abs(DrivingAngle)+pow(1.08, abs(DrivingAngle) - 20)-abs(driveTowardsPoint.integral*kI)-(derivative*kD)) #possible att stalla upp reaktiviteten
    else:
        DrivingAngle = -abs(0.01*abs(DrivingAngle)+pow(1.08, abs(DrivingAngle) - 20)+abs(driveTowardsPoint.integral*kI)+(derivative*kD))
