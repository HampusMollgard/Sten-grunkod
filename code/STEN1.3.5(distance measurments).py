...
if not devMode:
    ...
    hef_path = '/home/malte/Downloads/best9.hef'
    hef = HEF(hef_path)
...
def driveTowardsPoint(cX, cY, frame_height, frame_width):
    print(f"Driving towards point ({cX}, {cY})")
    ...
    if DrivingAngle > 0:
        ...
        print(f"DrivingAngle 1: {DrivingAngle}")
    else:
        DrivingAngle = -abs(0.01*abs(DrivingAngle)+pow(1.1, abs(DrivingAngle) - 45))
        print(f"DrivingAngle 2: {DrivingAngle}")
...
def normal_driving(runMotors, driveTowardsPoint, frame, annotated_image, mask, largest_contour, bottom_most_point, frame_height, frame_width):
    ...
    if top_most_point[1] < frame_height / 2:
            #Regular driving
        print("Topmost point is above the middle of the frame. Doing action A.")
        ...
        if top_most_point[1] < frame_height - 170:
            print("Topmost point is above 100 pixels from the bottom. Driving towards top_most_point.")
            driveTowardsPoint(x_start + mask_middle[1], y_start + mask_middle[0], frame_height, frame_width)
        else:
            print("Topmost point is within 100 pixels from the bottom. Driving towards cX, cY.")
            runMotors(200, 10)
            ...
            while top_most_point[1] > frame_height - 170:
                ...
                cv2.imshow("Processed camera", annotated_image)
...
def pointTurn(devMode, get_picture, angle):
    ...
    while time.time() - start_time < 0.9 * (angle / 90):
        ...
        if not ret:
            print("End of video or cannot read frame.")
            break
        if tempframe.shape[-1] == 4:
            tempframe = tempframe[:, :, :3]
        cv2.imshow("Processed camera", tempframe)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
    return
...
def get_distances_full(ser, send_speciall_data):
    send_speciall_data(3)
    line = ser.readline().decode().strip()  # Read a line, decode, and strip whitespace
    if line:  # If there's valid data
        values = line.split(",")  # Split by comma
        if len(values) == 4:  # Ensure we received four values
            forwardRight, forward, forwardLeft, rearRight = map(int, values)
            return forwardRight, forward, forwardLeft, rearRight
    return None, None, None, None

def get_distance_forward(ser, send_speciall_data):
    send_speciall_data(4)
    line = ser.readline().decode().strip()  # Read a line, decode, and strip whitespace
    if line:  # If there's valid data
        values = line.split(",")  # Split by comma
        if len(values) == 4:  # Ensure we received four values
...
while True:
    ...
    # Print the pixel value in the middle of the frame
    middle_pixel = hsv_frame[hsv_frame.shape[0] // 2, hsv_frame.shape[1] // 2]
    print(f"Middle pixel HSV value: {middle_pixel}")
    ...
    if red_contours:
        ...
        # Check if the bounding box is at least 90% of the width of the frame
                ...
                if confidence > 0.2:  # Filter low-confidence detections
                    ...
                    for corner in corners:
                        ...
                        if distance < min_distance:
                            ...
                            closest_index = (idx, idx2)  # Save the index of the closest detected box
                            print(f"Closest corner: {closest_corner}")
        ...
        if closest_corner:
            ...
            if (y_end - y_start) > 100:
                ...
                if class_names[closest_class] == "90 degree":  # 90 Degree
                    ...
                    if len(contours) >= 2:
                        print("Found two areas")
                        area1 = cv2.boundingRect(contours[0])
                        area2 = cv2.boundingRect(contours[1])
                        print(f"Area 1 coordinates: {area1}")
                        print(f"Area 2 coordinates: {area2}")
                        ...
                        print(f"Angle with vertical: {vertical_angle:.2f} degrees")
                ...
                elif class_names[closest_class] == "Green":  # Green
                    print("Error")
