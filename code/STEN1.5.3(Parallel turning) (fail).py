...
def driveTowardsPoint(cX, cY, frame_height, frame_width, speed):
    ...
    if True:
        ...
        if DrivingAngle > 0:
            DrivingAngle = abs(0.01*abs(DrivingAngle)+pow(1.1, abs(DrivingAngle) - 50))
        else:
            DrivingAngle = -abs(0.01*abs(DrivingAngle)+pow(1.1, abs(DrivingAngle) - 50))
        ...
        if top_most_point[1] < frame_height - 230:
            ...
            if contour_width < 0.35 * frame.shape[1]:
                ...
                if len(approx_corners) >= 4:
                    ...
                    pointTurnIntersection(devMode, get_picture, abs(angle - 90))
...
def pointTurn(devMode, get_picture, angle):
    ...
    #time.time() - start_time < 1.4 * (angle / 90)
    rotating = True
    while rotating:
        print("Running")
        ...
        height, width = mask.shape
        lower_middle_x_start = 0
        lower_middle_x_end = width
        ...
        # Convert the cropped mask to a 3-channel image
        cropped_mask_3channel = cv2.cvtColor(cropped_mask, cv2.COLOR_GRAY2BGR)

        # Resize the cropped mask to (640, 640, 3)
        cropped_mask_resized = cv2.resize(cropped_mask_3channel, (640, 640), interpolation=cv2.INTER_AREA)
        ...
        if contours:
            ...
            # Crop the top part (5 pixels in height)
            top_crop = mask[y:y + 5, x:x + w]
            top_contours, _ = cv2.findContours(top_crop, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

            # Crop the bottom part (5 pixels in height)
            bottom_crop = mask[y + h - 5:y + h, x:x + w]
            bottom_contours, _ = cv2.findContours(bottom_crop, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

            # Calculate moments for the top and bottom contours
            top_moments = [cv2.moments(contour) for contour in top_contours]
            bottom_moments = [cv2.moments(contour) for contour in bottom_contours]

            # Extract x-coordinates of centroids for top and bottom moments
            top_x_values = [int(m["m10"] / m["m00"]) for m in top_moments if m["m00"] != 0]
            bottom_x_values = [int(m["m10"] / m["m00"]) for m in bottom_moments if m["m00"] != 0]

            # Check if any top x value is close to any bottom x value
            for top_x in top_x_values:
                for bottom_x in bottom_x_values:
                    if abs(top_x - bottom_x) < 100:  # Threshold for closeness
                        print(f"Top x value {top_x} is close to bottom x value {bottom_x}")
                        rotating = False
                        break
            
            displayGUI(None, None, cropped_mask_resized)
                ...
                if class_names[closest_class] == "90 degree":  # 90 Degree
                        ...
                        while ballIndex < len(balls):
                            ...
                            if best_corners and best_center_x is not None:
                                ...
                                if distance_to_bottom < 10 and balls[ballIndex] != "Exit":
                                    runMotors(-0.1, 10)
                                    pointTurnIntersection(devMode, get_picture, 25)
                                    time.sleep(0.2)
                                    send_speciall_data(1)
                                    time.sleep(1)
                                    runMotors(50, 10)
                                    time.sleep(0.7)
                                    send_speciall_data(3)
                                    runMotors(0, 0)
                                    time.sleep(0.5)
                    ...
                    if TintersectionOkay:
                        if right and left:
                            ...
                            print("Forward")
                            runMotors(-700, 10)
                            forwardDrive(devMode, get_picture, 30)
                    else:
                        normal_driving(runMotors, driveTowardsPoint, frame, annotated_image, mask, largest_contour, bottom_most_point, frame_height, frame_width)
