...
def get_picture(dev_, camera_index):
    ...
    if dev_:
        ...
        frame_ = cv2.resize(frame_, (640, 640))
        frame_ = cv2.rotate(frame_, cv2.ROTATE_90_COUNTERCLOCKWISE)
        ret_ = True
    if frame_.shape[-1] == 4:
        frame_ = frame_[:, :, :3]
...
def driveTowardsPoint(cX, cY, frame_height, frame_width, speed):
    ...
    if cY > 400:
        if DrivingAngle > 0:
            DrivingAngle = abs(0.01*abs(DrivingAngle)+pow(1.1, abs(DrivingAngle) - 50)) #possible att stalla upp reaktiviteten
        else:
    ...
    if top_most_point[1] < frame_height / 2:
        ...
        if top_most_point[1] < frame_height - 230:
            ...
            if contour_width < 0.35 * frame.shape[1]:
                ...
                runMotors(-700, -6)
                forwardDrive(devMode, get_picture, 0.7)
runMotors(0, 0)
time.sleep(1.5)
...
def process_result(frame, detections, devMode, class_names, class_names_evaczone, displayGUI, Inference, get_picture, math_sign, send_speciall_data, runMotors, driveTowardsPoint, normal_driving, pointTurnIntersection, pointTurn, forwardDrive, red_contour_timeout):
    start = time.time()
            ...
            # Crop the mask
                ...
                # Get the coordinates of the two largest areas
                    ...
                    # Calculate the length between bottom_middle and center_x / center_y
                        ...
                        runMotors(-700, 10)
                        forwardDrive(devMode, get_picture, 0.4 + (0.2 * (distance_to_center / 200)))
                        if cross_product < 0:
                            runMotors(0.1, 10)
                            ...
                            pointTurn(devMode, get_picture, angle)
                        runMotors(-700, -6)
                        forwardDrive(devMode, get_picture, 0.3)
                        runMotors(0, 0)
...
def inference_thread(model_index):
    while True:
        frame = frame_queue.get()
        result = Inference(frame, model_index)
...
def processing_thread():
    while True:
        ...
        try:
            # Call the process_result function or wrap its critical sections
            process_result(frame, result, devMode, class_names, class_names_evaczone, displayGUI, Inference, get_picture, math_sign, send_speciall_data, runMotors, driveTowardsPoint, normal_driving, pointTurnIntersection, pointTurn, forwardDrive, red_contour_timeout)
...
def main():
    ...
    threading.Thread(target=inference_thread, args=(0,), daemon=True).start()
