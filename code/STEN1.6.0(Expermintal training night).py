...
hef_path = '/home/malte/Downloads/best19.hef'
hef = HEF(hef_path)
...
def driveTowardsPoint(cX, cY, frame_height, frame_width, speed): 
    ...
    driveTowardsPoint.prevDrivingAngle = DrivingAngle
    if DrivingAngle > 0:
        DrivingAngle = abs(0.01*abs(DrivingAngle)+pow(1.08, abs(DrivingAngle) - 30)-abs(driveTowardsPoint.integral*kI)-(derivative*kD))
    else:
        DrivingAngle = -abs(0.01*abs(DrivingAngle)+pow(1.08, abs(DrivingAngle) - 30)+abs(driveTowardsPoint.integral*kI)+(derivative*kD))
...
def normal_driving(runMotors, driveTowardsPoint, frame, annotated_image, mask, largest_contour, bottom_most_point, frame_height, frame_width):
    if button.is_pressed or buttonL.is_pressed or buttonR.is_pressed:
        drive_round_object()
        return
...
def drive_round_object():
    ...
    forwardDrive(get_picture, 40)
    
    pointTurn(get_picture, -90)
...
def pointTurn(get_picture, angle):
    send_secret_data(4, 140 * math_sign(angle))
    ...
    if not turning:
        pointTurnIntersection(get_picture, -math_sign(angle) * 140)
        pointTurn(get_picture,  -math_sign(angle) * 90)
    send_secret_data(0, 10)
    return
...
while True:
    ...
    lower_black = np.array([0, 0, 0])
    upper_black = np.array([180, 255, 80])
    mask_temp = cv2.inRange(hsv_frame, lower_black, upper_black)
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (21, 21))  # You can increase size if needed
    mask = cv2.morphologyEx(mask_temp, cv2.MORPH_CLOSE, kernel)
    ...
    if contours:
        # Find the largest contour by area
        # Find the largest and second largest contours by area
        sorted_contours = sorted(contours, key=cv2.contourArea, reverse=True)
        largest_contour = sorted_contours[0]
        cv2.drawContours(annotated_image, [largest_contour], -1, (0, 255, 0), 2)

        # Check if there is a second largest contour
        if len(sorted_contours) > 1:
            second_largest_contour = sorted_contours[1]
            area = 5000  # minimum area for second largest contour
            dist = 300    # maximum distance in px to consider "bordering"

            if cv2.contourArea(second_largest_contour) > area:
                # Calculate the minimum distance between the two contours
                # Find the minimum distance and the corresponding points
                min_dist_contour = float('inf')
                point1, point2 = None, None
                for p1 in largest_contour[:, 0]:
                    for p2 in second_largest_contour[:, 0]:
                        prevDist = cv2.norm(p1 - p2)
                        if prevDist < min_dist_contour:
                            min_dist_contour = dist
                            point1 = tuple(p1)
                            point2 = tuple(p2)
                print(f"Minimum distance between contours: {min_dist_contour}")
                if min_dist_contour < dist:
                    # Draw a line between the closest points of the two contours
                    cv2.line(annotated_image, point1, point2, (0, 255, 255), 5)
                    # Merge the two contours by concatenating their points and the connecting line
                    merged_contour = np.vstack((largest_contour, second_largest_contour))
                    # Optionally, add the connecting points as a tiny line contour for completeness
                    merged_contour = np.vstack((merged_contour, np.array([[point1], [point2]])))
                    # Use merged_contour as the new largest_contour for further processing
                    largest_contour = merged_contour
                    # Draw the merged contour
                    cv2.drawContours(annotated_image, [largest_contour], -1, (255, 0, 255), 2)
        ...
        candidate_points = [tuple(pt[0]) for pt in largest_contour if pt[0][1] == largest_contour[:, :, 1].max()]
        if candidate_points:
            bottom_most_point = min(candidate_points, key=lambda p: (p[0] - 320) ** 2 + (p[1] - 640) ** 2)
        else:
            bottom_most_point = tuple(largest_contour[largest_contour[:, :, 1].argmax()][0])
        cv2.circle(annotated_image, bottom_most_point, 10, (0, 0, 255), -1)
    ...
    for idx, det in enumerate(detections):
        ...
        for idx2, thing in enumerate(det):
            ...
            if (confidence > 0.40 and class_names[idx] == "90 degree") or confidence > 0.20 and class_names[idx] != "90 degree":  # Filter low-confidence detections
                        ...
                        while ballIndex < len(balls):
                            ...
                            if best_corners and best_center_x is not None:
                                ...
                                if distance_to_bottom < 10 and balls[ballIndex] != "Exit":
                                    ...
                                    if (balls[ballIndex] == "Live victim" or balls[ballIndex] == "Dead victim") and pickedBall:
                                        ballIndex += 1
                                                ...
                                                if confidence > 0.2:  # Filter low-confidence detections
                                                    ...
                                                    if w > 0.02 * frame_width:
                                                        if w > 0.9 * frame_width:
                                                            runMotors(-700, 10)
                                                            # Wait until both left and right button are pressed, or timeout of 3 seconds
                                                            wait_start = time.time()
                                                            while not (buttonL.is_pressed and buttonR.is_pressed):
                                                            ...
                                                            start = time.time()
                                                            break
                                                        driveTowardsBall(x_min + (w // 2.5), y_min + h // 2, frame_height, frame_width, 10)
                                                    else:
                                                        runMotors(0, 10)
                                                else:
                                                    runMotors(0, 10)
                                ...
                                elif balls[ballIndex] == "Exit" and (distance_to_bottom < 100): # or width_between_corners > 0.8 * frame_width
                                    runMotors(-700, 10)

                                    exitNotFound = True
                                    timeOut = time.time()
                                    buttonPressed = "None"
                                    while time.time() - timeOut < 4 and exitNotFound:
                                        runMotors(-700, 10)
                                        if buttonL.is_pressed:
                                            forwardDrive(get_picture, -100)
                                            runMotors(5, 10)
                                            ...
                                            for idx2, thing in enumerate(det):
                                                ...
                                                if confidence > 0.2:  # Filter low-confidence detections
                                                    ...
                                                    if class_names[idx] == "T-intersection" or class_names[idx] == "X-intersection":
                                                        ...
                                                        vertical_vector = (0, -1)  # Vertical line pointing upwards
                                                        line_vector = (intersection_box_middle_x - bottom_middle[0], intersection_box_middle_y - (bottom_middle[1] + 100))
                                                        dot_product_vertical = vertical_vector[0] * line_vector[0] + vertical_vector[1] * line_vector[1]
                                                        magnitude_vertical = math.sqrt(vertical_vector[0] ** 2 + vertical_vector[1] ** 2)
                                                        magnitude_line = math.sqrt(line_vector[0] ** 2 + line_vector[1] ** 2)
                                                        vertical_angle = math.degrees(math.acos(dot_product_vertical / (magnitude_vertical * magnitude_line)))

                                                        distance_to_center = math.sqrt((intersection_box_middle_x - bottom_middle[0]) ** 2 + (intersection_box_middle_y - (bottom_middle[1] + 100)) ** 2)

                                                        if (intersection_box_middle_x - bottom_middle[0]) > 50:
                                                            pointTurnIntersection(get_picture, vertical_angle)
                                ...
                                if time.time() - start > 10 and time.time() - start < 11:
                                    forwardDrive(get_picture, 100)
                                runMotors(0, 5)
                            displayGUI(None, None, annotated_image)
