...
class_names_evaczone = ["Dead victim", "Live victim", "Exit"]
...
if not devMode:
    ...
    hef_path2 = '/home/malte/Downloads/balls2.hef'
    hef2 = HEF(hef_path2)
...
def drive_round_object():
    ...
    while True:
        ...
        if contours:
            ...
            # Break the loop if the width of the rectangle exceeds 70% of the image width
            if w > 0.7 * frame.shape[1]:
                print("Largest black contour width exceeds 70% of the image width. Exiting loop.")
                break
    
    runMotors(-700, 10)
    forwardDrive(devMode, get_picture, 1.5)
    runMotors(-0.1, 10)
            ...
            if (y_end - y_start) > 120:
                ...
                if class_names[closest_class] == "90 degree":  # 90 Degree
                    ...
                    if object_width >= 0.9 * frame_width and object_height <= 0.4 * frame_height:  # Check width and height
                        ...
                        runMotors(0, 0)
                        #balls = ["Exit"]
                        balls = ["Live victim", "Live victim", "Dead victim", "Exit"]
                        ballIndex = 0
                                ...
                                if distance_to_bottom < 10 and balls[ballIndex] != "Exit":
                                    ...
                                    if balls[ballIndex] == "Live victim":
                                            ...
                                            if green_contours:
                                                ...
                                                if w > 0.02 * frame_width:
                                                    if w > 0.9 * frame_width:
                                                        ...
                                                        ballIndex += 1
                                                        break
                                                    driveTowardsPoint(x + w // 2, y + h // 2, frame_height, frame_width, 10)
                                                else:
                                                    runMotors(0, 10)
                                            else:
                                                runMotors(0, 10)
                                        ...
                                        while True:
                                            ...
                                            displayGUI(None, None, annotated_image)
                                elif balls[ballIndex] == "Exit" and distance_to_bottom < 40:
                                    runMotors(-15, 10)
                                    time.sleep(1.5)
                                    send_speciall_data(5)
                                    time.sleep(1.5)
