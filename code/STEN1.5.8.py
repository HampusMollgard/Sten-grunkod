...
cv2.resizeWindow("STEN 11", 320, 320)  # Set the size to 800x600
...
def pointTurn(get_picture, angle):
    ...
    if not(time.time() - start_time < 10):
...
while True:
    ...
    # Sort contours by area (descending) and take up to 3 largest
    largest_contours = sorted(filtered_contours, key=cv2.contourArea, reverse=True)[:3]

    bottom_points = []
    for contour in largest_contours:
        cv2.drawContours(annotated_image, [contour], -1, (0, 255, 0), 2)
        # Find the bottom-most point for this contour
        bottom_point = tuple(contour[contour[:, :, 1].argmax()][0])
        bottom_points.append(bottom_point)
        cv2.circle(annotated_image, bottom_point, 15, (0, 0, 255), -1)

    # Optionally, you can pick the lowest of all bottom points as the main bottom_most_point
    if bottom_points:
        bottom_most_point = max(bottom_points, key=lambda pt: pt[1])
        cv2.circle(annotated_image, bottom_most_point, 20, (255, 0, 0), 3)
    else:
        bottom_most_point = (0, 320)
