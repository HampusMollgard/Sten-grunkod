...
def driveTowardsPoint(cX, cY, frame_height, frame_width, speed): 
    ...
    driveTowardsPoint.prevDrivingAngle = DrivingAngle
    if DrivingAngle > 0:
        DrivingAngle = abs(0.01*abs(DrivingAngle)+pow(1.08, abs(DrivingAngle) - 40)+abs(driveTowardsPoint.integral*kI)-(derivative*kD))
    else:
        DrivingAngle = -abs(0.01*abs(DrivingAngle)+pow(1.08, abs(DrivingAngle) - 40)-abs(driveTowardsPoint.integral*kI)+(derivative*kD))
...
while True:
    ...
    for idx, det in enumerate(detections):
        ...
        for idx2, thing in enumerate(det):
            ...
            if (confidence > 1 and class_names[idx] == "90 degree") or confidence > 0.60 and class_names[idx] != "90 degree":  # Filter low-confidence detections
                ...
                if class_names[idx] != "Green" and class_names[idx] != "90 degree":  # Ignore class with index 1
                    ...
                    if class_names[idx] == "T-intersection":
                        corners_okay = all(
